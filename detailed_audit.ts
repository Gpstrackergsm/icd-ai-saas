...
console.log('                         DETAILED CARDIOLOGY AUDIT - CASE BY CASE REVIEW');
console.log('═'.repeat(100));
console.log();
...
allCases.forEach(tc => {
    ...
    console.log('─'.repeat(100));
    console.log(`CASE ${tc.num}: ${status}`);
    console.log('─'.repeat(100));
    console.log(`Clinical Scenario: ${tc.text}`);
    console.log();
    console.log(`Expected Rationale: ${tc.rationale}`);
    console.log();

    // Primary diagnosis
    console.log(`PRIMARY DIAGNOSIS:`);
    console.log(`  Expected: ${tc.expectedPrimary}`);
    console.log(`  Actual:   ${actualPrimary} ${primaryMatch ? '✅' : '❌'}`);
    if (result.primary) {
    ...
    if (tc.expectedSecondary.length === 0 && actualSecondary.length === 0) {
        console.log(`  Expected: (none)`);
        console.log(`  Actual:   (none) ✅`);
    } else if (tc.expectedSecondary.length === 0) {
        console.log(`  Expected: (none)`);
        console.log(`  Actual:   ${actualSecondary.join(', ')} ⚠️ Extra codes generated`);
    } else {
        console.log(`  Expected: ${tc.expectedSecondary.join(', ')}`);
        console.log(`  Actual:   ${actualSecondary.length > 0 ? actualSecondary.join(', ') : '(none)'} ${secondaryMatch ? '✅' : '❌'}`);

        // Show labels for secondary codes
        if (result.secondary.length > 0) {
            result.secondary.forEach((code, idx) => {
                const isExpected = tc.expectedSecondary.includes(code.code);
                console.log(`    ${idx + 1}. ${code.code} - ${code.label} ${isExpected ? '' : '⚠️'}`);
            });
        }
...
if (correctCount === 40) {
    console.log('🎉 PERFECT SCORE! ALL 40 CASES PASSED!');
} else {
    console.log(`⚠️  ${40 - correctCount} case(s) need review`);
}
