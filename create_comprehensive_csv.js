...
// Read data
const cases = JSON.parse(fs.readFileSync('icd_1000_cases.json', 'utf-8'));
...
cases.forEach((testCase) => {
    ...
    // Clean text for CSV
    const scenario = testCase.clinical_text.replace(/"/g, '""');
    const expected = testCase.expected_codes.join(', ');
    const generated = result.codes;

    // Calculate missing and extra
    const expectedSet = new Set(testCase.expected_codes);
