...
casesRaw.forEach((caseText, index) => {
    ...
    if (context.conditions.respiratory?.mechanicalVent?.present) { // Strict check (removed duration check to be safe as per user rule "Vent = Resp Failure")
    ...
    if (caseText.includes('Drug Use: Yes') && !caseText.toLowerCase().includes('abuse')) {
        if (codes.includes('F19.10')) messages.push('FAIL: Drug Use: Yes mapped to F19.10 (Should be Z72.2)');
        if (!codes.includes('Z72.2') && !codes.some(c => c.startsWith('F'))) messages.push('FAIL: Drug Use: Yes missing code');
    }
    ...
    if (caseText.includes('Stage/Depth: Stage 3') && caseText.includes('Type: Diabetic')) {
        const hasBoneExposed = caseText.toLowerCase().includes('bone exposed') || context.conditions.diabetes?.ulcerSeverity === 'bone';
        ...
        if (!l97) {
            ...
            if (!l97.endsWith('4')) messages.push(`FAIL: Diabetic Stage 3 with Bone Exposed mapped to ${l97} (Expected suffix 4)`);
        } else {
            ...
            if (!l97.endsWith('3')) messages.push(`FAIL: Diabetic Stage 3 mapped to ${l97} (Expected suffix 3)`);
        }
    }

    // 4. Bone Exopsed -> L97.x94
    if (context.conditions.diabetes?.ulcerSeverity === 'bone' || caseText.toLowerCase().includes('bone exposed')) {
        const l97 = codes.find(c => c.startsWith('L97'));
        if (l97 && !l97.endsWith('4')) messages.push(`FAIL: Bone Exposed mapped to ${l97} (Expected suffix 4)`);
    }
    ...
    if (codes.includes('J44.0') || codes.includes('J44.1')) {
        ...
        if (j96Index !== -1 && j96Index > j44Index) messages.push('FAIL: Sequencing J44 > J96');
        if (j18Index !== -1 && j18Index > j44Index) messages.push('FAIL: Sequencing J44 > Pneumonia');
    }
    ...
    if (hasE22 && hasN18) messages.push('FAIL: E1x.22 and N18.x coexist (Rule 2 Violation)');

    // 7. CKD-Diabetes Conflict: E1x.21 + E1x.22 -> FAIL
    const hasE21 = codes.some(c => /^E1[0-9]\.21/.test(c));
    if (hasE21 && hasE22) messages.push('FAIL: E1x.21 and E1x.22 coexist (Rule 1 Violation)');
