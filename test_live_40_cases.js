...
async function testAllCases() {
    ...
    console.log('LIVE PRODUCTION API TEST - 40 CARDIOLOGY CASES');
    console.log('Testing: https://www.icd-10-cm.online/api/encode-structured');
    console.log('='.repeat(80));
    ...
    for (const testCase of TEST_CASES) {
        try {
            const response = await callLiveAPI(testCase.narrative);

            // Extract codes from response
            let generatedCodes = [];
            if (response.success && response.data && response.data.codes) {
                generatedCodes = response.data.codes.map(c => c.code);
            } else if (response.data && Array.isArray(response.data)) {
                generatedCodes = response.data.map(c => c.code);
            }

            // Check if all expected codes are present
            const missing = testCase.expected.filter(e => !generatedCodes.includes(e));
            const extra = generatedCodes.filter(g => !testCase.expected.includes(g));
            ...
            if (isPass) {
                ...
                failures.push({ id: testCase.id, expected: testCase.expected, generated: generatedCodes, missing, extra });
                failed++;
            }
            ...
            console.log(`❌ Case ${testCase.id}: ERROR - ${error.message}`);
            failures.push({ id: testCase.id, error: error.message });
            failed++;
        }
    }
    ...
    if (failures.length > 0) {
        ...
        console.log('\n🎉 ALL TESTS PASSED ON LIVE PRODUCTION! 🎉');
    }
}
