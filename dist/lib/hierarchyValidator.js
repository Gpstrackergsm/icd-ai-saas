...
function isBillable(code) {
    ...
    if (threeCharBillable.has(code))
        return true;
    // Most codes need 4+ characters to be billable
    return stripped.length >= 4;
}
function requiresSeventh(code) {
    return /^T[0-9]/.test(code) || code.includes('X');
}
function validateHierarchy(sequence) {
    const warnings = [];
    const filtered = [];
    ...
    filtered.forEach((entry) => {
        ...
        const hasMoreSpecific = filtered.some((other) => other.code !== entry.code &&
...
function processRemainingRules(filtered, warnings) {
