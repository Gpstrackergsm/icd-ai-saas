...
exports.applyConsistencyRules = applyConsistencyRules;
exports.validateFinalOutput = validateFinalOutput;
function applyConsistencyRules(codes, input) {
    const violations = [];
    const applied_fixes = [];
    ...
    if (validatedCodes.some(c => c.code === 'A41.01') &&
        ...
        validatedCodes = validatedCodes.map(c => c.code === 'A41.01' ? { ...c, code: 'A41.9', label: 'Sepsis, unspecified organism' } : c);
    ...
    if (lowerInput.includes('sepsis')) {
        if (lowerInput.includes('mrsa') && !validatedCodes.some(c => c.code === 'A41.02')) {
            ...
            if (hasWrongSepsis) {
                validatedCodes = validatedCodes.map(c => c.code.startsWith('A41') ? { ...c, code: 'A41.02', label: 'Sepsis due to mrsa' } : c);
        ...
        if (lowerInput.includes('e. coli') && !validatedCodes.some(c => c.code === 'A41.51')) {
            ...
            if (hasWrongSepsis) {
                validatedCodes = validatedCodes.map(c => c.code.startsWith('A41') ? { ...c, code: 'A41.51', label: 'Sepsis due to e_coli' } : c);
        ...
        if (lowerInput.includes('viral') && lowerInput.includes('sepsis')) {
            ...
            if (hasBadViral) {
                validatedCodes = validatedCodes.map(c => c.code === 'A41.89' ? { ...c, code: 'A41.9', label: 'Sepsis, unspecified organism' } : c);
...
function validateFinalOutput(codes, input) {
