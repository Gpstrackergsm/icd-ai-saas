...
const { context: ctx1, errors: err1 } = (0, parser_1.parseInput)(test1);
if (err1.length > 0) {
    console.log('\n❌ Parse Errors:', err1);
}
else {
    ...
    codes1.forEach((code, i) => {
        ...
        console.log(`${i + 1}. ${code} - ${codeObj === null || codeObj === void 0 ? void 0 : codeObj.label}`);
    });
...
const { context: ctx2, errors: err2 } = (0, parser_1.parseInput)(test2);
if (err2.length > 0) {
    console.log('\n❌ Parse Errors:', err2);
}
else {
    console.log('\n✅ Parsed Successfully');
    const result2 = (0, engine_1.runStructuredRules)(ctx2);
