...
const guidelineReferences_js_1 = require("./guidelineReferences.js");
/**
 * Generate comprehensive clinical rationale for each code in the sequence
 */
function generateRationale(codes, warnings) {
    const rationales = [];
    codes.forEach((c, index) => {
        const specificityNotes = [];
        ...
        const guideline = (0, guidelineReferences_js_1.getGuidelineForCode)(c.code, c.label);
...
function generateSummary(codes, warnings) {
    const parts = [];
