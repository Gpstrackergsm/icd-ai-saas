...
exports.resolveNeoplasm = resolveNeoplasm;
function resolveNeoplasm(text) {
    ...
    if (/cancer|malignancy|carcinoma|sarcoma|tumor|neoplasm/.test(lower)) {
        ...
        // Check for specific secondary sites
        if (/metastatic.*to.*brain|metastasis.*to.*brain/.test(lower))
            secondarySite = 'brain';
        if (/metastatic.*to.*bone|metastasis.*to.*bone/.test(lower))
            secondarySite = 'bone';
        if (/metastatic.*to.*liver|metastasis.*to.*liver/.test(lower))
            secondarySite = 'liver';
        if (/metastatic.*to.*lung|metastasis.*to.*lung/.test(lower))
            secondarySite = 'lung';
        ...
        if (secondarySite === 'brain')
            secondary_codes.push({ code: 'C79.31', label: 'Secondary malignant neoplasm of brain', type: 'metastasis' });
        if (secondarySite === 'bone')
            secondary_codes.push({ code: 'C79.51', label: 'Secondary malignant neoplasm of bone', type: 'metastasis' });
        if (secondarySite === 'liver')
            secondary_codes.push({ code: 'C78.7', label: 'Secondary malignant neoplasm of liver and intrahepatic bile duct', type: 'metastasis' });
        if (secondarySite === 'lung' && site !== 'lung')
            secondary_codes.push({ code: 'C78.00', label: 'Secondary malignant neoplasm of unspecified lung', type: 'metastasis' });
