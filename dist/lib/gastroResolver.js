...
exports.resolveGastro = resolveGastro;
function resolveGastro(text) {
    ...
    if (/pancreatitis/.test(lower)) {
        ...
        if (/alcohol/.test(lower))
            code = 'K85.2';
        if (/biliary|gallstone/.test(lower))
            code = 'K85.1';
        return {
            code,
    ...
    if (/gastritis/.test(lower)) {
        ...
        if (isAcute)
            code = bleeding ? 'K29.01' : 'K29.00';
        else if (isChronic)
            code = bleeding ? 'K29.51' : 'K29.50';
        else
            code = bleeding ? 'K29.71' : 'K29.70';
        return {
            code,
            label: 'Gastritis',
