...
exports.resolveTrauma = resolveTrauma;
function resolveTrauma(text) {
    const lower = text.toLowerCase();
    const warnings = [];
    const secondary_codes = [];
    // Encounter detection
    let encounter = 'initial';
    if (/subsequent|follow[- ]up|healing/.test(lower))
        encounter = 'subsequent';
    if (/sequela|late effect/.test(lower))
        encounter = 'sequela';
    ...
    if (/fracture|broken/.test(lower)) {
        ...
        if (encounter === 'initial') {
            ...
            if (/delayed/.test(lower))
                char7 = 'G';
            if (/nonunion/.test(lower))
                char7 = 'K';
            if (/malunion/.test(lower))
                char7 = 'P';
        }
        ...
        // Rib
            ...
            if (laterality === 'right')
                baseCode = `S22.31X${char7}`;
            if (laterality === 'left')
                baseCode = `S22.32X${char7}`;
