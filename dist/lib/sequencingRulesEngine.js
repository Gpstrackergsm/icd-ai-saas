...
exports.applySequencingRules = applySequencingRules;
/**
 ...
 */
const postProceduralSepsisRule = {
    name: 'Post-Procedural Sepsis Sequencing',
    priority: 1,
    applies: (codes) => codes.some(c => c.code === 'T81.44XA'),
    sequence: (codes) => {
        const t8144 = codes.find(c => c.code === 'T81.44XA');
        const organDysfunction = codes.filter(c => c.code.startsWith('J95.8') || // Respiratory failure post-procedure
        ...
        const sourceInfection = codes.filter(c => c.code.startsWith('J18') || // Pneumonia
            c.code.startsWith('N39.0') // UTI
        );
        const remaining = codes.filter(c => c.code !== 'T81.44XA' &&
...
const sepsisWithShockRule = {
    ...
    sequence: (codes) => {
        ...
        const localizedInfection = codes.filter(c => c.code.startsWith('N39.0') || // UTI
            c.code.startsWith('J18') || // Pneumonia
            c.code.startsWith('N10') // Pyelonephritis
        );
        const remaining = codes.filter(c => c !== sepsis &&
            c !== shock &&
...
const injuryWithPainRule = {
    ...
    sequence: (codes) => {
        const injury = codes.filter(c => (c.code.startsWith('S') || c.code.startsWith('T')) &&
            !c.code.startsWith('T81.44') // Exclude post-procedural sepsis
        );
        const pain = codes.find(c => c.code === 'G89.11');
        const externalCause = codes.filter(c => c.code.startsWith('W') ||
            c.code.startsWith('X') ||
            c.code.startsWith('Y'));
        const remaining = codes.filter(c => !injury.includes(c) &&
            c !== pain &&
 ...
 */
const etiologyBeforeManifestation = {
    ...
    sequence: (codes) => {
        ...
        const remaining = codes.filter(c => !diabetes.includes(c) &&
            !ckd.includes(c) &&
 ...
 */
const externalCauseLastRule = {
    name: 'External Cause Codes Last',
    priority: 10, // Lower priority, applies broadly
    applies: (codes) => codes.some(c => c.code.startsWith('W') ||
        c.code.startsWith('X') ||
        c.code.startsWith('Y')),
    sequence: (codes) => {
        const externalCause = codes.filter(c => c.code.startsWith('W') ||
 ...
 */
function applySequencingRules(codes) {
    const rationale = [];
    const errors = [];
    const warnings = [];
    if (codes.length === 0) {
 ...
 */
function validateSepsisSequencing(codes, errors, warnings) {
 ...
 */
function validateEtiologyManifestation(codes, errors, warnings) {
 ...
 */
function validateExternalCause(codes, errors, warnings) {
    const externalCauseIndices = codes
        .map((c, i) => ({ code: c, index: i }))
        .filter(({ code }) => code.code.startsWith('W') ||
    ...
    if (externalCauseIndices.length > 0) {
        ...
        const hasCodesAfterExternal = codes.slice(lastExternalIndex + 1).some(c => !c.code.startsWith('W') &&
            !c.code.startsWith('X') &&
            !c.code.startsWith('Y'));
