 ...
 */
function calculateConfidence(codes, warnings) {
    const factors = [];
    ...
    const otherWarnings = warnings.filter(w => !/laterality|7th character/i.test(w)).length;
    if (otherWarnings > 0) {
...
function generateExplanation(confidence, factors) {
    if (confidence >= 90) {
