...
// CATEGORY HANDLERS
// -----------------------------------------------------------------------------
function parseHTN_HF_CKD(story, lines) {
    ...
    if (lower.includes('heart failure') || lower.includes('chf')) {
        ...
        if (lower.includes('systolic'))
            lines.push('HF Type: Systolic');
        else if (lower.includes('diastolic'))
            lines.push('HF Type: Diastolic');
        else if (lower.includes('combined'))
            lines.push('HF Type: Combined');
    }
    // CKD
    if (lower.includes('ckd') || lower.includes('kidney disease')) {
        lines.push('CKD: Yes');
        if (lower.includes('stage 3'))
            lines.push('CKD Stage: 3');
        else if (lower.includes('stage 4'))
            lines.push('CKD Stage: 4');
        else if (lower.includes('stage 5'))
            lines.push('CKD Stage: 5');
        else if (lower.includes('esrd'))
            lines.push('CKD Stage: ESRD');
    }
}
function parseInternalDM(story, lines) {
    const lower = story.toLowerCase();
    // Diabetes Type
    if (lower.includes('type 1'))
        lines.push('Diabetes: Type 1');
    else
        lines.push('Diabetes: Type 2'); // Default to Type 2 if not specified, or if Type 2 explicitly stated
    // Complications
    ...
    if (lower.includes('foot ulcer') || lower.includes('ulcer')) {
        ...
        if (lower.includes('muscle'))
            lines.push('Ulcer Depth: Muscle');
        else if (lower.includes('bone'))
            lines.push('Ulcer Depth: Bone');
        else if (lower.includes('fat'))
            lines.push('Ulcer Depth: Fat');
    }
...
function parseTrauma(story, lines) {
    const lower = story.toLowerCase();
    // Injury Type
    if (lower.includes('fracture'))
        lines.push('Injury: Fracture');
    else if (lower.includes('laceration') || lower.includes('open wound'))
        lines.push('Injury: Laceration');
    else if (lower.includes('burn'))
        lines.push('Injury: Burn');
    // Body Part
    if (lower.includes('femur'))
        lines.push('Site: Femur');
    else if (lower.includes('ankle'))
        lines.push('Site: Ankle');
    else if (lower.includes('foot') || lower.includes('feet'))
        lines.push('Site: Foot');
    else if (lower.includes('finger'))
        lines.push('Site: Finger');
    else if (lower.includes('chest'))
        lines.push('Site: Chest');
    else if (lower.includes('abdomen') || lower.includes('abdominal'))
        lines.push('Site: Abdomen');
    // Laterality
    if (lower.includes('left'))
        lines.push('Laterality: Left');
    else if (lower.includes('right'))
        lines.push('Laterality: Right');
    // Encounter
    lines.push('Encounter: Initial'); // Default for trauma in this dataset context usually
}
function parseUlcers(story, lines) {
    const lower = story.toLowerCase();
    // Pressure Ulcer
    if (lower.includes('pressure ulcer') || lower.includes('bedsore')) {
        ...
        if (lower.includes('stage 1'))
            ...
            lines.push('Stage: 4');
        else if (lower.includes('unstageable'))
            lines.push('Stage: Unstageable');
        if (lower.includes('sacral') || lower.includes('sacrum'))
            lines.push('Site: Sacral');
        else if (lower.includes('heel'))
            lines.push('Site: Heel');
        else if (lower.includes('buttock'))
            lines.push('Site: Buttock');
    }
}
function parseOncology(story, lines) {
    const lower = story.toLowerCase();
...
function parsePsych(story, lines) {
    ...
    if (lower.includes('alcohol')) {
        ...
        if (lower.includes('dependence'))
            lines.push('Status: Dependence');
        else if (has(lower, 'abuse'))
            lines.push('Status: Abuse');
    }
}
function has(text, term) { return text.includes(term); }
function parseZCodes(story, lines) {
    const lower = story.toLowerCase();
    if (lower.includes('screening')) {
        lines.push('Type: Screening');
        if (lower.includes('colonoscopy'))
            lines.push('Screening For: Colon Cancer');
        else if (lower.includes('mammogram'))
            lines.push('Screening For: Breast Cancer');
    }
    if (lower.includes('aftercare')) {
        lines.push('Type: Aftercare');
        if (lower.includes('surgery'))
            lines.push('After: Surgery');
    }
}
function parsePeds(story, lines) {
    const lower = story.toLowerCase();
    if (lower.includes('otitis media')) {
        ...
        if (lower.includes('left'))
            ...
            lines.push('Laterality: Right');
        else if (lower.includes('bilateral'))
            lines.push('Laterality: Bilateral');
    }
    if (lower.includes('croup')) {
        lines.push('Condition: Croup');
    }
}
function parseSepsisAndPulmonary(story, lines) {
...
// -----------------------------------------------------------------------------
function parseClinicalStory(story, age, gender, encounterType, category) {
    const lines = [];
    lines.push(`Age: ${age}`);
...
function generateCategorySpecificCodes(structuredInput, category) {
    const codes = [];
    const lines = structuredInput.split('\n').map(l => l.toLowerCase());
    // Helper to check content
    const has = (text) => lines.some(l => l.includes(text.toLowerCase()));
    if (category === 'TRAUMA') {
        // --- TRAUMA LOGIC ---
        // S72.301A - Fracture of shaft of right femur, initial
        if (has('Injury: Fracture') && has('Site: Femur') && has('Laterality: Right'))
            codes.push('S72.301A');
        else if (has('Injury: Fracture') && has('Site: Femur') && has('Laterality: Left'))
            codes.push('S72.302A');
        // S91.001A - Open wound of right ankle
        else if (has('Injury: Laceration') && has('Site: Ankle') && has('Laterality: Right'))
            codes.push('S91.001A');
        else if (has('Injury: Laceration') && has('Site: Ankle') && has('Laterality: Left'))
            codes.push('S91.002A');
        // S91.301A - Open wound of right foot
        else if (has('Injury: Laceration') && has('Site: Foot') && has('Laterality: Right'))
            codes.push('S91.301A');
        else if (has('Injury: Laceration') && has('Site: Foot') && has('Laterality: Left'))
            codes.push('S91.302A');
        // S61.210A - Laceration of finger
        else if (has('Injury: Laceration') && has('Site: Finger') && has('Laterality: Right'))
            codes.push('S61.210A');
        else if (has('Injury: Laceration') && has('Site: Finger') && has('Laterality: Left'))
            codes.push('S61.211A');
        // S21.90XA - Open wound of chest
        else if (has('Injury: Laceration') && has('Site: Chest'))
            codes.push('S21.90XA');
        // T21.21XA - Burn of chest (second degree)
        else if (has('Injury: Burn') && has('Site: Chest'))
            codes.push('T21.21XA');
        // S31.109A - Open wound of abdomen
        else if (has('Injury: Laceration') && has('Site: Abdomen'))
            codes.push('S31.109A');
        // Fallback if generic fracture
        if (has('Injury: Fracture') && codes.length === 0)
            codes.push('S72.90XA'); // Unspecified fracture code as fallback
    }
        ...
        if (has('History of Cancer: Yes')) {
            if (has('Cancer: Breast'))
                codes.push('Z85.3');
            else if (has('Cancer: Lung'))
                codes.push('Z85.118');
            else if (has('Cancer: Colon'))
                codes.push('Z85.038');
            else
                codes.push('Z85.9');
        }
        ...
        if (has('Condition: Preeclampsia')) {
            if (has('Severity: Severe'))
                codes.push('O14.10'); // Severe pre-eclampsia, unspecified trimester
            else
                codes.push('O14.90'); // Unspecified pre-eclampsia
        }
        ...
        if (has('Condition: Stroke')) {
            if (has('Deficit: Hemiplegia'))
                codes.push('I69.359'); // Hemiplegia and hemiparesis following cerebral infarction
            else if (has('Deficit: Aphasia'))
                codes.push('I69.320'); // Aphasia following cerebral infarction
            else
                codes.push('I63.9'); // Cerebral infarction, unspecified (Acute stroke)
        ...
        if (has('Condition: Epilepsy')) {
            if (has('Status: Intractable'))
                codes.push('G40.919'); // Epilepsy, unspecified, intractable, without status epilepticus
            else
                codes.push('G40.909'); // Epilepsy, unspecified, not intractable, without status epilepticus
        }
    }
    else if (category === 'PSYCH') {
        // --- PSYCH LOGIC ---
        if (has('Condition: Depression')) {
            if (has('Type: Major') && has('Pattern: Recurrent'))
                codes.push('F33.9'); // Major depressive disorder, recurrent, unspecified
            else
                codes.push('F32.9'); // Major depressive disorder, single episode, unspecified
        }
        if (has('Condition: Anxiety')) {
            codes.push('F41.9'); // Anxiety disorder, unspecified
        }
        if (has('Condition: Alcohol Use')) {
            if (has('Status: Dependence'))
                codes.push('F10.20'); // Alcohol dependence, uncomplicated
            else if (has('Status: Abuse'))
                codes.push('F10.10'); // Alcohol abuse, uncomplicated
        }
    }
    else if (category === 'ZCODES') {
        // --- ZCODES LOGIC ---
        if (has('Type: Screening')) {
            if (has('Screening For: Colon Cancer'))
                codes.push('Z12.11'); // Encounter for screening for malignant neoplasm of colon
            else if (has('Screening For: Breast Cancer'))
                codes.push('Z12.31'); // Encounter for screening mammogram
        }
        if (has('Type: Aftercare')) {
            if (has('After: Surgery'))
                codes.push('Z48.815'); // Encounter for surgical aftercare following surgery on the digestive system (common default) or Z48.0
        ...
        if (has('Condition: Otitis Media')) {
            if (has('Laterality: Right'))
                codes.push('H66.91');
            else if (has('Laterality: Left'))
                codes.push('H66.92');
            else if (has('Laterality: Bilateral'))
                codes.push('H66.93');
            else
                codes.push('H66.90'); // Otitis media, unspecified, unspecified ear
        }
...
function processCases() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('CASE ')) {
            if (caseNumber > 0 && currentCase.clinicalStory) {
                ...
                const structuredInput = parseClinicalStory(currentCase.clinicalStory, currentCase.age, currentCase.gender, currentCase.encounterType, currentCase.category);
                try {
    ...
    processedCases.forEach(c => {
        if (!categoryStats[c.category])
            categoryStats[c.category] = { total: 0, withCodes: 0 };
        categoryStats[c.category].total++;
        if (c.codes.length > 0)
            categoryStats[c.category].withCodes++;
    });
    console.log('\nCategory Breakdown:');
