...
casesRaw.forEach((caseText, index) => {
    ...
    if (hasDrugUse && !hasAbuse) {
        ...
        if (forbiddenFCodes.length > 0) {
            ...
            // Check specifically:
            if (caseText.includes('Drug Use: Yes') && !fixedCodes.includes('Z72.2'))
                fixedCodes.push('Z72.2');
            if (caseText.includes('Alcohol: Yes') && !fixedCodes.includes('Z72.89'))
                fixedCodes.push('Z72.89');
        }
    }
    // [2] DIABETIC ULCER
    ...
    if (fixedCodes.some(c => c.endsWith('621')) && !isPressure) { // E10.621 or E11.621
        ...
        if (lowerText.includes('ankle'))
            site = '3';
        else if (lowerText.includes('heel'))
            site = '4';
        ...
        if (lowerText.includes('bone') || lowerText.includes('stage 4'))
            depth = '4';
        else if (lowerText.includes('muscle') || lowerText.includes('stage 3'))
            depth = '3';
        else if (lowerText.includes('fat') || lowerText.includes('subcutaneous') || lowerText.includes('stage 2'))
            depth = '2';
        else if (lowerText.includes('stage 1') || lowerText.includes('skin'))
            depth = '1';
    ...
    // [5] SEPSIS
    if (caseText.includes('Sepsis: Yes') || ((_b = (_a = context.conditions.infection) === null || _a === void 0 ? void 0 : _a.sepsis) === null || _b === void 0 ? void 0 : _b.present)) {
    ...
    if (n18Codes.length > 1) {
        ...
        const getRank = (code) => {
            if (code.includes('N18.6'))
                return 6;
            if (code.includes('N18.5'))
                return 5;
            if (code.includes('N18.4'))
                return 4;
            if (code.includes('N18.3'))
                return 3;
            if (code.includes('N18.2'))
                return 2;
            if (code.includes('N18.1'))
                return 1;
    ...
    const pnaCodes = fixedCodes.filter(c => c.startsWith('J12') ||
        c.startsWith('J15') ||
        c.startsWith('J18') ||
        c.startsWith('J69') ||
        c.startsWith('J95'));
    if (pnaNo) {
        ...
        if (pnaCodes.length === 0) {
            ...
            if (org.includes('viral'))
                newCode = 'J12.9';
            else if (org.includes('mrsa'))
                newCode = 'J15.212';
            else if (org.includes('pseudomonas'))
                newCode = 'J15.1';
            else if (org.includes('e. coli') || org.includes('e.coli'))
                newCode = 'J15.5';
            else if (org.includes('streptococcus'))
                newCode = 'J15.4';
    ...
    if (isCorrect)
        ...
        fixedCount++;
    let summary = `Gender: ${(_c = context.demographics) === null || _c === void 0 ? void 0 : _c.gender}, Age: ${(_d = context.demographics) === null || _d === void 0 ? void 0 : _d.age}`;
    if (context.conditions.diabetes)
        summary += `, DM ${context.conditions.diabetes.type}`;
    if ((_e = context.conditions.ckd) === null || _e === void 0 ? void 0 : _e.stage)
        summary += `, CKD ${context.conditions.ckd.stage}`;
