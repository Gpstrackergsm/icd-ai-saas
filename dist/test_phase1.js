...
Septic Shock: Yes
`;
const test1 = (0, parser_1.parseInput)(test1Input);
const test1Validation = (0, validator_1.validateContext)(test1.context);
const test1Result = (0, engine_1.runStructuredRules)(test1.context);
console.log('Validation:', test1Validation.valid ? '✅ PASSED' : '❌ FAILED');
if (!test1Validation.valid)
...
console.log('\nActual Codes:');
if (test1Result.primary)
    console.log(`  PRIMARY: ${test1Result.primary.code} - ${test1Result.primary.label}`);
test1Result.secondary.forEach(c => console.log(`  SECONDARY: ${c.code} - ${c.label}`));
const hasR6521 = ((_a = test1Result.primary) === null || _a === void 0 ? void 0 : _a.code) === 'R65.21' || test1Result.secondary.some(c => c.code === 'R65.21');
...
Ulcer Stage: Stage 3
`;
const test2 = (0, parser_1.parseInput)(test2Input);
const test2Validation = (0, validator_1.validateContext)(test2.context);
const test2Result = (0, engine_1.runStructuredRules)(test2.context);
console.log('Validation:', test2Validation.valid ? '✅ PASSED' : '❌ FAILED');
if (!test2Validation.valid)
    console.log('Errors:', test2Validation.errors);
console.log('\nExpected Codes:');
console.log('  - L89.153 (Pressure ulcer of sacral region, stage 3)');
console.log('\nActual Codes:');
if (test2Result.primary)
    console.log(`  PRIMARY: ${test2Result.primary.code} - ${test2Result.primary.label}`);
test2Result.secondary.forEach(c => console.log(`  SECONDARY: ${c.code} - ${c.label}`));
const hasL89153 = ((_b = test2Result.primary) === null || _b === void 0 ? void 0 : _b.code) === 'L89.153' || test2Result.secondary.some(c => c.code === 'L89.153');
...
External Cause: Fall
`;
const test3 = (0, parser_1.parseInput)(test3Input);
const test3Validation = (0, validator_1.validateContext)(test3.context);
const test3Result = (0, engine_1.runStructuredRules)(test3.context);
console.log('Validation:', test3Validation.valid ? '✅ PASSED' : '❌ FAILED');
if (!test3Validation.valid)
...
console.log('\nActual Codes:');
if (test3Result.primary)
    console.log(`  PRIMARY: ${test3Result.primary.code} - ${test3Result.primary.label}`);
test3Result.secondary.forEach(c => console.log(`  SECONDARY: ${c.code} - ${c.label}`));
const hasS72301A = ((_c = test3Result.primary) === null || _c === void 0 ? void 0 : _c.code) === 'S72.301A' || test3Result.secondary.some(c => c.code === 'S72.301A');
...
Sepsis: Yes
`;
const test4 = (0, parser_1.parseInput)(test4Input);
const test4Validation = (0, validator_1.validateContext)(test4.context);
...
`;
const test5 = (0, parser_1.parseInput)(test5Input);
const test5Validation = (0, validator_1.validateContext)(test5.context);
console.log('Expected: HARD STOP error');
