...
function runAudit() {
    ...
    for (const testCase of testCases) {
        totalTests++;
        try {
            const { context } = (0, parser_1.parseInput)(testCase.clinicalData);
            ...
            const finalResult = (0, validator_enhanced_1.validateFinalOutput)(validated.codes, testCase.clinicalData);
            const systemCodes = finalResult.codes.map(c => c.code);
            const expectedCodes = testCase.expectedCodes;
            // Check if codes match
            const codesMatch = systemCodes.length === expectedCodes.length &&
