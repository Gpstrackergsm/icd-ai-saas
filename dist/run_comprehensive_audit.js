    ...
    try {
        ...
        if (codesChanged || finalResult.errors.length > 0) {
            ...
            if (codesChanged) {
                ...
                if (removed.length > 0)
                    errorLog += `    - Removed: ${removed.join(', ')}\n`;
                if (added.length > 0)
                    errorLog += `    - Added: ${added.join(', ')}\n`;
            }
            if (finalResult.errors.length > 0) {
                errorLog += `  ERRORS:\n`;
...
results.forEach(r => {
    if (r.changed) {
        const removed = r.originalCodes.filter((c) => !r.newCodes.includes(c));
        const added = r.newCodes.filter((c) => !r.originalCodes.includes(c));
        removed.forEach((c) => {
