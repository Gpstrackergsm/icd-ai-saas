...
/**
 * MEDICAL CODING AUDIT - HOSPITAL 1000 CASES
 * 
 * Processes cases and audits results as a Senior Medical Coder
 */
...
function auditCodes(category: string, clinicalStory: string, systemCodes: string[]): any {
    const audit = {
        correct: false,
    ...
    if (story.includes('e. coli') && story.includes('uti') && story.includes('septic shock')) {
        ...
        if (systemCodes.includes('A41.51') && systemCodes.includes('N39.0') && systemCodes.includes('R65.21')) {
            ...
            if (!systemCodes.includes('A41.51')) audit.issues.push('Missing A41.51 (E. coli sepsis)');
            if (!systemCodes.includes('N39.0')) audit.issues.push('Missing N39.0 (UTI)');
            if (!systemCodes.includes('R65.21')) audit.issues.push('Missing R65.21 (Septic shock)');
        }
    }
