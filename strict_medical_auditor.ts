...
/**
 * MEDICAL CODING AUDITOR - STRICT CORRECTION ENGINE
 * 
 * Re-audits all cases and applies strict ICD-10-CM guidelines
 */
...
interface AuditResult {
    ...
    systemCodes: string[];
    correctedCodes: string[];
    correctionsMade: string[];
    rulesApplied: string[];
    verdict: 'CORRECT' | 'CORRECTED';
}

function auditSepsisCase(story: string, systemCodes: string[]): any {
...
function auditHTNHFCKDCase(story: string, systemCodes: string[]): any {
    const corrections: string[] = [];
    const rulesApplied: string[] = [];
    let correctedCodes = [...systemCodes];
...
async function main() {
    ...
    for (let i = 0; i < Math.min(caseBlocks.length, 1000); i++) {
        ...
        try {
            const { context } = parseInput(parsedInput);
            const result = runStructuredRules(context);

            const systemCodes: string[] = [];
            if (result.primary) systemCodes.push(result.primary.code);
            systemCodes.push(...result.secondary.map(c => c.code));

            if (systemCodes.length === 0) continue;
...
function parseClinicalStory(story: string, age: number, gender: string, encounterType: string): string {
    const lines: string[] = [`Age: ${age}`, `Gender: ${gender}`, `Encounter Type: ${encounterType}`];
    const lowerStory = story.toLowerCase();
    ...
    if (lowerStory.includes('e. coli')) lines.push('Organism: E. coli');
    else if (lowerStory.includes('mrsa')) lines.push('Organism: MRSA');

    if (lowerStory.includes('hypertension') || lowerStory.includes('htn')) lines.push('Hypertension: Yes');
    if (lowerStory.includes('heart failure') || lowerStory.includes(' hf ')) {
        lines.push('Heart Failure: Systolic');
        if (lowerStory.includes('acute on chronic')) lines.push('Heart Failure Acuity: Acute on chronic');
        else if (lowerStory.includes('acute')) lines.push('Heart Failure Acuity: Acute');
    }
