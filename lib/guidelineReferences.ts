...
export const GUIDELINE_DATABASE: Record<string, GuidelineReference> = {
...
export function getGuidelineForCode(code: string, context?: string): GuidelineReference | undefined {
    // Sepsis
    if (/^A4[01]/.test(code)) return GUIDELINE_DATABASE['sepsis'];
    if (/^T81\.44/.test(code)) return GUIDELINE_DATABASE['post_procedural_sepsis'];

    // Diabetes
    if (/^E(08|09|10|11|13)/.test(code)) {
        if (context === 'manifestation') return GUIDELINE_DATABASE['diabetes_manifestation'];
        return GUIDELINE_DATABASE['diabetes'];
    }

    // Hypertension
    if (/^I1[123]/.test(code)) {
        if (/^I13/.test(code)) return GUIDELINE_DATABASE['hypertensive_combination'];
        return GUIDELINE_DATABASE['hypertension'];
    }

    // MI
    if (/^I21/.test(code)) return GUIDELINE_DATABASE['mi'];

    // COPD/Respiratory
    if (/^J44/.test(code)) return GUIDELINE_DATABASE['copd'];
    if (/^J96/.test(code)) return GUIDELINE_DATABASE['respiratory_failure'];

    // CKD
    if (/^N18/.test(code)) return GUIDELINE_DATABASE['ckd'];

    // Obstetrics
    if (/^O/.test(code)) return GUIDELINE_DATABASE['obstetrics'];

    // Neoplasm
    if (/^C/.test(code)) {
        if (/^C7[789]/.test(code)) return GUIDELINE_DATABASE['metastatic'];
        return GUIDELINE_DATABASE['neoplasm'];
    }

    // Injury/Trauma
    if (/^[ST]/.test(code)) {
        if (/fracture/i.test(context || '')) return GUIDELINE_DATABASE['fracture'];
        return GUIDELINE_DATABASE['injury_7th'];
    }

    // Pain
    if (/^G89/.test(code)) return GUIDELINE_DATABASE['pain'];

    // External Cause
    if (/^[VWXY]/.test(code)) return GUIDELINE_DATABASE['external_cause'];

    // Z codes
    if (/^Z12/.test(code)) return GUIDELINE_DATABASE['screening'];
    if (/^Z85/.test(code)) return GUIDELINE_DATABASE['history'];
    if (/^Z/.test(code)) return GUIDELINE_DATABASE['z_codes'];
