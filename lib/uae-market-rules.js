...
const TEMPORAL_EXCLUSIONS = {
    historical: /\b(last week|last month|previously|prior|history of|past|in 20\d{2}|earlier)\b/i,
    future: /\b(scheduled|next week|next month|will do|upcoming|planned)\b/i,
    pending: /\b(pending|awaiting|sent for|ordered)\b/i
};

const UNCERTAINTY_EXCLUSIONS = /\b(likely|suspected|possible|query|rule out|r\/o|probable|consider|equivocal|indeterminate|false positive)\b/i;
...
const APPROVED_DIAGNOSTIC_TESTS = {
    ...
    'blood culture': {
        positivePattern: /blood culture.*(?:positive|grew|growth).*e\.?\s*coli|e\.?\s*coli.*blood culture.*(?:positive|grew)/i,
...
const APPROVED_PROCEDURES = {
    'incision and drainage': {
        patterns: [
            ...
            /i\s*&\s*d\s+(?:of|performed|completed)/i,
