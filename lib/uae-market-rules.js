...
const ALLOWED_REASON_TYPES = Object.freeze({
    ...
    EXPLICIT_CLINICAL_PRESENTATION: 'EXPLICIT_CLINICAL_PRESENTATION'
});
...
// ============================================================================
// APPROVED CLINICAL PRESENTATIONS
// ============================================================================

const APPROVED_CLINICAL_PRESENTATIONS = {
    'strangulated inguinal hernia': {
        patterns: [
            // Pattern 1: Irreducible + severity markers
            /irreducible.*inguinal.*(?:swelling|bulge|mass)/i,
            // Pattern 2: Strangulated + inguinal
            /strangulated.*inguinal.*(?:hernia|swelling|bulge)/i,
            // Pattern 3: Incarcerated + inguinal
            /incarcerated.*inguinal.*(?:hernia|swelling|bulge)/i
        ],
        severityMarkers: [
            /\b(severe|tenderness|tender|pain|vomiting|nausea)\b/i
        ],
        diagnosis: { code: 'K40.30', description: 'Unilateral inguinal hernia, with obstruction, without gangrene, not specified as recurrent' },
        requiresSeverityMarker: true,
        reasonType: ALLOWED_REASON_TYPES.EXPLICIT_CLINICAL_PRESENTATION,
        reason: 'Clinical presentation of strangulated inguinal hernia - UAE clinical presentation override'
    },
    'uncomplicated inguinal hernia': {
        patterns: [
            /inguinal.*(?:swelling|bulge|mass|hernia)/i,
            /groin.*(?:swelling|bulge|mass)/i
        ],
        excludePatterns: [
            /irreducible/i,
            /strangulated/i,
        ...
        diagnosis: { code: 'K40.90', description: 'Unilateral inguinal hernia, without obstruction or gangrene, not specified as recurrent' },
        requiresSeverityMarker: false,
        reasonType: ALLOWED_REASON_TYPES.EXPLICIT_CLINICAL_PRESENTATION,
        reason: 'Clinical presentation of uncomplicated inguinal hernia - UAE clinical presentation override'
    }
};

/**
 * Detect approved clinical presentations with strict filters
 */
function detectApprovedClinicalPresentations(narrative) {
    const detected = [];
    const debug = {
        triggers: [],
        negations: [],
    ...
    for (const [presentationName, config] of Object.entries(APPROVED_CLINICAL_PRESENTATIONS)) {
        // Check temporal exclusion
        if (hasTemporalExclusion(narrative)) {
            debug.temporality.push(`${presentationName}: temporal exclusion detected`);
            continue;
        }

        // Check uncertainty
        if (hasUncertainty(narrative)) {
            debug.uncertainty.push(`${presentationName}: uncertainty marker detected`);
            continue;
        }

        // Check if presentation pattern matches
        const matched = config.patterns.some(pattern => pattern.test(narrative));
        if (!matched) continue;

        debug.triggers.push(`${presentationName}: presentation pattern matched`);

        // Check exclude patterns (for uncomplicated hernia)
        if (config.excludePatterns) {
            const hasExclusion = config.excludePatterns.some(pattern => pattern.test(narrative));
            if (hasExclusion) {
                debug.negations.push(`${presentationName}: exclusion pattern detected`);
                continue;
            }
        }

        // Check severity markers if required (for strangulated hernia)
        if (config.requiresSeverityMarker && config.severityMarkers) {
            const hasSeverityMarker = config.severityMarkers.some(pattern => pattern.test(narrative));
            if (!hasSeverityMarker) {
                debug.severityCheck.push(`${presentationName}: no severity marker found`);
                continue;
            }
            debug.severityCheck.push(`${presentationName}: severity marker confirmed`);
        }
        ...
        detected.push({
            ...
            evidenceSource: narrative.match(config.patterns[0])?.[0] || presentationName
        });

        // Only return first match (most specific)
        break;
    }
...
const APPROVED_PROCEDURES = {
    ...
    'hernia repair': {
        patterns: [
            ...
            /herniorrhaphy/i,
            /herniotomy/i,
            /laparoscopic.*hernia/i,
            /open.*hernia.*repair/i
        ],
        decliningPattern: /(?:recommended|declined|refused|patient declined|consider for|planned for|scheduled for)/i,
        diagnosis: { code: 'K40.90', description: 'Unilateral inguinal hernia, without obstruction or gangrene, not specified as recurrent' },
        strangulatedCode: { code: 'K40.30', description: 'Unilateral inguinal hernia, with obstruction, without gangrene, not specified as recurrent' },
        reasonType: ALLOWED_REASON_TYPES.APPROVED_PROCEDURE_IMPLIED_DX,
        reason: 'Hernia repair procedure - UAE approved procedure-implied diagnosis'
    },
    'dialysis': {
        patterns: [
...
function detectApprovedProcedures(narrative) {
    ...
    for (const [procedureName, config] of Object.entries(APPROVED_PROCEDURES)) {
        ...
        // Check for strangulated hernia (hernia repair only)
        if (procedureName === 'hernia repair' && config.strangulatedCode) {
            if (/\b(strangulated|incarcerated|with obstruction)\b/i.test(narrative)) {
                diagnosisCode = config.strangulatedCode.code;
                description = config.strangulatedCode.description;
                debug.triggers.push(`${procedureName}: strangulated/incarcerated variant → ${diagnosisCode}`);
            }
        }
...
function checkUAEOverride(narrative, marketProfile) {
    ...
    // Check approved clinical presentations
    const presentationResult = detectApprovedClinicalPresentations(narrative);
    certDebug.triggers.push(...presentationResult.debug.triggers);
    certDebug.negations.push(...presentationResult.debug.negations);
    certDebug.temporality.push(...presentationResult.debug.temporality);
    certDebug.uncertainty.push(...presentationResult.debug.uncertainty);
    if (presentationResult.debug.severityCheck) {
        certDebug.severityCheck = presentationResult.debug.severityCheck;
    }

    if (presentationResult.detected.length > 0) {
        const presentation = presentationResult.detected[0];

        // GATEKEEPER CHECK
        if (!canAutoCodeUAE(presentation.reasonType)) {
            return {
                shouldOverride: false,
                reason: 'GATEKEEPER BLOCK: Reason type not approved for AUTO_CODE',
                _debug: {
                    uaeCert: certDebug,
                    reasonType: presentation.reasonType,
                    allowed: false
                }
            };
        }

        certDebug.matchedRules.push(`Clinical Presentation: ${presentation.presentation} → ${presentation.diagnosis.code}`);

        return {
            shouldOverride: true,
            diagnoses: [{
                code: presentation.diagnosis.code,
                description: presentation.diagnosis.description,
                reasonType: presentation.reasonType,
                evidenceSource: presentation.evidenceSource,
                auditTrailText: presentation.reason
            }],
            reasons: [presentation.reason],
            metadata: {
                uae_override: true,
                presentation_name: presentation.presentation,
                evidence_type: 'EXPLICIT_CLINICAL_PRESENTATION'
            },
            _debug: {
                uaeCert: certDebug,
                reasonType: presentation.reasonType,
