...
function parseMetadata(rawTitle) {
    ...
    // 3. Cleanup "UPD M", "COM", and other noise commonly found in raw ICD descriptions
    // Remove "UPD M", "COM", "M" at end of string or isolated
    description = description.replace(/\b(UPD\s*M|COM|M)\b/g, '').replace(/\s+/g, ' ').trim();
