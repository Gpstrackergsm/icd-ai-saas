...
export function resolveCardiovascular(text: string): CardiovascularResolution | undefined {
    ...
    const getCkdCode = (): string => {
        ...
        else if (ckdStage === 5 || hasEsrd) n18Code = 'N18.6';
        return n18Code;
    };
    ...
    if (hasHypertension) {
        if (hasHf && hasCkd) {
            ...
            let code = 'I13.10';
            if (hasHf) {
                if (ckdStage === 5 || hasEsrd) code = 'I13.2';
                else code = 'I13.0';
            } else {
                if (ckdStage === 5 || hasEsrd) code = 'I13.11';
                else code = 'I13.10';
            }

            // Add secondary codes
            if (hasHf) secondary_codes.push({ code: getHfCode(), label: 'Heart failure', type: 'manifestation' });
            if (hasCkd) secondary_codes.push({ code: getCkdCode(), label: `Chronic kidney disease, stage ${ckdStage || 'unspecified'}`, type: 'manifestation' });
    ...
    if (/infarction|\bmi\b|heart attack|stemi|nstemi/.test(lower)) {
        ...
        if (isStemi) {
            ...
            if (/lateral/.test(lower)) code = 'I21.29';
            if (/posterior/.test(lower)) code = 'I21.29'; // Often coded as other sites
            if (/left main/.test(lower)) code = 'I21.01';
