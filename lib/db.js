...
const upsertUser = ({ id, email, stripeCustomerId, subscriptionStatus, currentPeriodEnd, lastPaymentDate, plan }) => {
  const normalizedEmail = email ? email.toLowerCase() : null;
  const now = Math.floor(Date.now() / 1000);
  ...
  if (existing) {
    db.prepare(
      `UPDATE users SET stripe_customer_id = COALESCE(?, stripe_customer_id), subscription_status = COALESCE(?, subscription_status), current_period_end = COALESCE(?, current_period_end), plan = COALESCE(?, plan) WHERE email = ?`
    ).run(stripeCustomerId, normalizedStatus, currentPeriodEnd, plan || existing?.plan, normalizedEmail);
    if (lastPaymentDate) {
  ...
  db.prepare(
    `INSERT INTO users (id, email, stripe_customer_id, subscription_status, current_period_end, created_at, last_payment_date, plan) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    userId,
    normalizedEmail,
...
const jsonApi = {
  ...
  upsertUser({ id, email, stripeCustomerId, subscriptionStatus, currentPeriodEnd, lastPaymentDate }) {
