...
const upsertUser = ({ id, email, stripeCustomerId, subscriptionStatus, currentPeriodEnd }) => {
  ...
  if (existing) {
    ...
    ).run(stripeCustomerId, subscriptionStatus, currentPeriodEnd, normalizedEmail);
  ...
  ).run(userId, normalizedEmail, stripeCustomerId || null, subscriptionStatus || null, currentPeriodEnd || null, now);
  if (normalizedEmail) {
    return getUserByEmail(normalizedEmail);
  }
  return {
...
const setSubscriptionStatus = (email, status, currentPeriodEnd) => {
  if (!email) return null;
  upsertUser({ email, subscriptionStatus: status, currentPeriodEnd });
};
...
const recordSubscription = ({ userId, stripeSubscriptionId, status, priceId }) => {
  ...
  if (existing) {
    ...
    ).run(status, priceId, now, stripeSubscriptionId);
    return;
  }
  db.prepare(
    `INSERT INTO subscriptions (user_id, stripe_subscription_id, status, price_id, updated_at) VALUES (?, ?, ?, ?, ?)`
  ).run(userId, stripeSubscriptionId, status, priceId, now);
};
