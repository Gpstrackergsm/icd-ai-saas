...
ensureColumn("users", "trial", "INTEGER DEFAULT 1");
...
const upsertUser = ({
  ...
  const statusValue = normalizedStatus ? normalizedStatus.toLowerCase() : existing?.subscription_status || null;
  const planValue = plan || existing?.plan || null;
  const trialValue = typeof trial === "boolean" ? (trial ? 1 : 0) : existing?.trial ?? 1;
  const searchCountValue =
    typeof searchCount === "number" && !Number.isNaN(searchCount)
      ? searchCount
  ...
  if (existing) {
    db.prepare(
      `UPDATE users SET
        ...
        plan = COALESCE(?, plan),
        trial = COALESCE(?, trial),
        search_count = COALESCE(?, search_count)
      WHERE email = ?`
    ).run(
      stripeCustomerId,
      statusValue,
  ...
  return {
    ...
    subscription_status: statusValue || null,
...
const incrementUserSearchCount = (email) => {
  if (!email) return null;
  const normalized = email.toLowerCase();
  const user = getUserByEmail(normalized);
  if (!user) return null;
  const nextCount = (user.search_count || 0) + 1;
  db.prepare("UPDATE users SET search_count = ? WHERE email = ?").run(nextCount, normalized);
  return getUserByEmail(normalized);
};

const disableTrialForUser = (email) => {
  if (!email) return null;
  const normalized = email.toLowerCase();
  db.prepare("UPDATE users SET trial = 0 WHERE email = ?").run(normalized);
  return getUserByEmail(normalized);
};
...
const sqliteMetrics = {
  ...
  trials() {
    return db.prepare("SELECT COUNT(*) as total FROM users WHERE trial = 1").get()?.total || 0;
  },
  conversionRate() {
  ...
  mrr() {
    const activeSubscriptions = db
      .prepare("SELECT stripe_customer_id FROM subscriptions WHERE isActive = 1")
      .all();
      ...
      .prepare(
        `SELECT customer_id, amount_paid
         ...
         WHERE status = 'succeeded' AND paid_at = (
           SELECT MAX(p2.paid_at) FROM payments p2 WHERE p2.customer_id = p.customer_id AND p2.status = 'succeeded'
         )`
      )
      .all();

    const paymentMap = new Map(latestPayments.map((p) => [p.customer_id, p.amount_paid || 0]));

    return activeSubscriptions.reduce((sum, sub) => sum + (paymentMap.get(sub.stripe_customer_id) || 0), 0);
  },
...
const jsonApi = {
  ...
  incrementUserSearchCount(email) {
    if (!email) return null;
    const normalized = email.toLowerCase();
    const user = jsonHelpers.findUser(normalized);
    if (!user) return null;
    user.search_count = (user.search_count || 0) + 1;
    persistJsonStore();
    return user;
  },
...
const jsonMetrics = {
  ...
  trials() {
    return jsonStore.users.filter((u) => u.trial === 1).length;
  },
  conversionRate() {
    const trials = this.trials();
    if (!trials) return 0;
  ...
  mrr() {
    const paymentMap = new Map(
      jsonStore.payments
        ...
        .map((p) => [p.customer_id, p.amount_paid || 0])
    );

    return jsonStore.subscriptions
      .filter((s) => s.isActive === 1)
      .reduce((sum, sub) => sum + (paymentMap.get(sub.stripe_customer_id) || 0), 0);
  },
  recentPayments(limit = 10) {
...
module.exports = {
  ...
  incrementUserSearchCount: useJsonStore
    ? jsonApi.incrementUserSearchCount.bind(jsonApi)
    : incrementUserSearchCount,
  disableTrialForUser: useJsonStore ? jsonApi.disableTrialForUser.bind(jsonApi) : disableTrialForUser,
