...
const upsertUser = ({ id, email, stripeCustomerId, subscriptionStatus, currentPeriodEnd, lastPaymentDate }) => {
  const normalizedEmail = email ? email.toLowerCase() : null;
  const now = Math.floor(Date.now() / 1000);
  ...
  db.prepare(
    `INSERT INTO users (id, email, stripe_customer_id, subscription_status, current_period_end, created_at, last_payment_date) VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(
    userId,
    normalizedEmail,
