...
function medicalAudit(caseId: number, input: string, codes: string[]): AuditResult {
    const errors: string[] = [];
    ...
    if (lower.includes('cancer') || lower.includes('neoplasm')) {
        ...
        if (isActive && !hasCCode) {
            let cCode = 'C80.1';
            if (lower.includes('breast')) cCode = 'C50.919';
            else if (lower.includes('lung')) cCode = 'C34.90';
            else if (lower.includes('colon')) cCode = 'C18.9';
            else if (lower.includes('prostate')) cCode = 'C61';
    ...
    if (lower.includes('pressure') && lower.includes('ulcer')) {
        ...
        if (!hasL89 || correctedCodes.includes('L89.90')) {
            ...
            if (lower.includes('sacral') || lower.includes('sacrum')) {
                ...
                if (lower.includes('stage 4')) ulcerCode = 'L89.624';
                    ...
                    if (lines[j].includes('NO CODES') || lines[j].includes('NO CODABLE')) {
                        ...
                        for (let k = j; k < Math.min(j + 10, lines.length); k++) {
                            if (lines[k].includes('[PRIMARY]') || lines[k].includes('[SECONDARY]')) {
                                const code = lines[k].split(']')[1]?.trim().split(' ')[0];
