...
function convertToStructuredFormat(primary: string, secondary: string, domain: string): string {
    ...
    if (s.includes('stage 3b')) structured += 'CKD Stage: 3b\n';
    else if (s.includes('stage 3a')) structured += 'CKD Stage: 3a\n';
    else if (s.includes('stage 3')) structured += 'CKD Stage: 3\n';
    else if (s.includes('stage 2')) structured += 'CKD Stage: 2\n';
    else if (s.includes('stage 4')) structured += 'CKD Stage: 4\n';
    else if (s.includes('stage 5')) structured += 'CKD Stage: 5\n';
    ...
    if (p.includes('acute kidney failure') && p.includes('superimposed on chronic kidney disease')) {
        ...
        if (p.includes('stage 3')) structured += 'CKD Stage: 3\n';
    }
...
async function runVerification() {
    const testData = JSON.parse(
        fs.readFileSync('./data/icd10_test_cases_1000.json', 'utf-8')
    ) as TestCase[];
