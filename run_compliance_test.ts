...
function inferDomain(input: string): string {
    ...
    if (lower.includes('sepsis') || lower.includes('septic')) return 'Sepsis';
    if (lower.includes('hypertension') || lower.includes('heart failure')) return 'Cardiology';
    ...
    if (lower.includes('infection') || lower.includes('uti')) return 'Infection';
    return 'Other';
}

function validateClinicalConsistency(codes: string[], input: string): string[] {
...
async function runComplianceTest() {
    ...
    const violationMap: Map<string, number[]> = new Map();
    const domainStats: Map<string, { total: number, passed: number, failed: number }> = new Map();
    ...
    for (const testCase of cases) {
        try {
            ...
            const status: 'PASS' | 'FAIL' = hasCodes && violations.length === 0 ? 'PASS' : 'FAIL';
    ...
    const violations: Violation[] = Array.from(violationMap.entries()).map(([type, caseIds]) => ({
        type,
        caseIds,
