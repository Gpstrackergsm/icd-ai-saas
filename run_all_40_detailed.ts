...
console.log('╔════════════════════════════════════════════════════════════════════════════════╗');
console.log('║           COMPREHENSIVE CARDIOLOGY TEST SUITE - DETAILED RESULTS              ║');
console.log('║                              40 CASES                                          ║');
console.log('╚════════════════════════════════════════════════════════════════════════════════╝\n');
...
cases.forEach((caseText, idx) => {
    ...
    console.log(`┌─ CASE ${caseNum} ${'─'.repeat(78 - `CASE ${caseNum} `.length)}`);
    console.log(`│`);
    console.log(`│ ${caseText.substring(0, 76)}`);
    if (caseText.length > 76) {
        console.log(`│ ${caseText.substring(76, 152)}`);
    }
    console.log(`│`);

    if (allCodes.length === 0) {
        console.log(`│ ⚠️  NO CODES GENERATED`);
    } else {
        allCodes.forEach((code, i) => {
            const position = i === 0 ? '🔴 PRIMARY  ' : '⚪ SECONDARY';
            console.log(`│ ${i + 1}. ${position} │ ${code!.code.padEnd(10)} │ ${code!.label.substring(0, 45)}`);
        });
        passCount++;
    }
...
console.log(`✅ Cases with codes generated: ${passCount}/40 (${((passCount / 40) * 100).toFixed(1)}%)`);
console.log(`📝 Total codes generated: ${totalCodes}`);
console.log(`📈 Average codes per case: ${(totalCodes / 40).toFixed(1)}`);

if (passCount === 40) {
    console.log('\n🎉 SUCCESS: All 40 cases generated codes!');
} else {
    console.log(`\n⚠️  WARNING: ${40 - passCount} cases failed to generate codes`);
}
