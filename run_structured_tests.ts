...
function loadExpectedCodes(): Map<number, string[]> {
...
function inferDomain(input: string): string {
    ...
    if (lower.includes('diabetes') || lower.includes('insulin')) return 'Diabetes';
    if (lower.includes('sepsis') || lower.includes('septic shock')) return 'Sepsis';
    if (lower.includes('copd') || lower.includes('asthma') || lower.includes('pneumonia') || lower.includes('respiratory')) return 'Respiratory';
    if (lower.includes('hypertension') || lower.includes('heart failure') || lower.includes('atrial')) return 'Cardiology';
    if (lower.includes('ckd') || lower.includes('kidney') || lower.includes('renal')) return 'Renal';
    if (lower.includes('cancer') || lower.includes('neoplasm') || lower.includes('chemotherapy')) return 'Oncology';
    if (lower.includes('labor') || lower.includes('gestation') || lower.includes('pregnancy')) return 'OB/GYN';
    if (lower.includes('cirrhosis') || lower.includes('ascites') || lower.includes('liver')) return 'Liver/GI';
    if (lower.includes('infection') && lower.includes('urinary')) return 'Infectious';
    if (lower.includes('depression') || lower.includes('anxiety')) return 'Psychiatry';
    if (lower.includes('burn') || lower.includes('ulcer') && !lower.includes('diabetes')) return 'Dermatology';
    if (lower.includes('alzheimer') || lower.includes('dementia') || lower.includes('stroke')) return 'Neurology';
    return 'Unknown';
}
    ...
    for (const testCase of cases) {
        ...
        try {
            ...
            if (status === 'FAIL') {
                ...
                if (generatedCodes.length === 0) errorType = 'No codes generated';
                else if (expected.length === 0) errorType = 'No expected codes';
                else if (generatedCodes.length < expected.length) errorType = 'Missing codes';
                else if (generatedCodes.length > expected.length) errorType = 'Extra codes';
                else errorType = 'Wrong codes';
