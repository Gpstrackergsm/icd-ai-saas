...
console.log('COMPREHENSIVE CARDIOLOGY MODULE TEST - DETAILED RESULTS');
console.log('='.repeat(100));
...
TEST_CASES.forEach(testCase => {
    ...
    const missing = testCase.expected.filter(e => !generated.includes(e));
    const extra = generated.filter(g => !testCase.expected.includes(g));
    const isPass = missing.length === 0 && extra.length === 0;

    console.log(`\n${'─'.repeat(100)}`);
    console.log(`CASE ${testCase.id}: ${isPass ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`${'─'.repeat(100)}`);
    console.log(`Input: "${testCase.narrative.substring(0, 90)}..."`);
    console.log(`\nGenerated Codes (in sequence):`);
    ...
    if (codes.length === 0) {
        ...
        codes.forEach((c, i) => {
            ...
            const expectedMarker = testCase.expected.includes(c.code) ? '✓' : '✗';
            console.log(`  ${i + 1}. [${position}] ${c.code} - ${c.label.substring(0, 60)} ${expectedMarker}`);
        });
    }

    console.log(`\nExpected: [${testCase.expected.join(', ')}]`);
    console.log(`Got:      [${generated.join(', ')}]`);
