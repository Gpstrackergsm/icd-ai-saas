// LEVEL 1: ICD-10-CM Audit Authority Engine
// Builds ABOVE frozen LEVEL 0 (AUTO_EXCLUDE)

const lookupDetail = require('../lib/icd-dictionary.js').lookupDetail;

// ============================================================================
// LEVEL 1: ICD-10-CM CODE MAPPING DICTIONARY
// ============================================================================
const ICD10_MAPPING = {
  // Renal
  ...
  'severe sepsis': { codes: ['A41.9', 'R65.20'], descriptions: ['Sepsis, unspecified organism', 'Severe sepsis without septic shock'] },
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (lower.includes('diabetic foot ulcer') && !isNegated('diabetic foot ulcer')) {
      ...
      // Check for separate diabetes and foot ulcer
    ...
    // ========================================================================
    // LEVEL 1: CODE MAPPING AND QUERY GENERATION
    // ========================================================================
    const codes = [];
    const queries = [];
    ...
    for (const diagnosis of detectedDiagnoses) {
      ...
      if (mapping.query) {
        ...
        // Multiple codes (e.g., severe sepsis)
